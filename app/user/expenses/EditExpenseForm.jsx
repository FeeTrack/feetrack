...
export default function EditExpenseForm({ profile, expense, expenseTypes, staff, currentSession, onSubmit, onCancel }) {
  const [state, formAction, pending] = useActionState(updateExpenseAction, { error: null });
  const [clientErrors, setClientErrors] = useState({})

  const [selectedExpType, setSelectedExpType] = useState(
    expenseTypes.find(et => et.id === expense.expense_heads.id) || {}
    )
  const [selectedStaff, setSelectedStaff] = useState(
    staff.find(s => s.id === expense.salary_expenses?.[0].staff?.id) || {}
    )
  const [showSalary, setShowSalary] = useState(false)

  const [formData, setFormData] = useState({
    expenseType: expense.expense_heads.id,
    selectedStaff: expense.salary_expenses?.[0].staff?.id || '',
    amount: expense.amount,
    expenseDate: expense.expense_date,
    description: expense.description ? expense.description : ''
  });

  console.log(formData.selectedStaff)
    ...
    <>
      <div>
        <div className='flex-1 overflow-y-auto max-h-full'>
          <form action={formAction} onSubmit={handleSubmit} className="space-y-3 w-full" id="editExpenseForm">
                ...
                <div>
                    <div className='flex justify-between items-center'>
                        ...
                        {profile.role === 'admin' && selectedStaff.id && (
                            <button type='button' className='primary-btn text-xs mb-1' onClick={() => setShowSalary(prev => !prev)}>
