...
export async function updateStudentFeeStructure({student, currentSession}) {
    ...
    for (const cfs of classFeeStructure) {
        ...
        if (duration === 'once_per_student') {
            ...
            if (student.month_fee_from === 'adm_date') {
                ...
                return { error: 'Unknown month_fee_from value: ' + student.month_fee_from };
            }
            ...
            return { error: 'Unexpected fee head duration: ' + duration };
        }
    }
    ...
    const { data: invoices, error: invUpdateError } = await supabase
        .from('invoices')
        .upsert(allInvoicePayloads, { onConflict: ['school_id', 'student_id', 'pay_period'] })
        .select('id, pay_period');

    if (invUpdateError) {
        return { error: 'Failed to create invoices. ' + invUpdateError.message };
    }
    ...
    const { data: invItems, error: itemUpdateError } = await supabase
        .from('invoice_items')
        .upsert(finalInvoiceItemsPayloads, { onConflict: ['student_id', 'fee_head_id', 'period_key', 'is_late_fee'] })
    ...
    if (itemUpdateError) {
        await supabase
            ...
            .eq('session_id', currentSession.id);
        return { error: 'Failed to create invoice items. ' + itemUpdateError.message };
    }
    ...
    if (periodStartEnd.length > 0) {
        ...
        // Delete existing period_start_end entries for this student's invoice items
        await supabase
            .from('period_start_end')
            .delete()
            .in('invoice_item_id', Array.from(admDateInvoiceItemsIdMap.values()));
        
        // Insert new period_start_end entries    
    ...
    if (student.route_id) {
        ...
        const {error: transInvItemInsertErr} = await supabase
            .from('invoice_items')
            .upsert(finalTransInvItemsPayloads, { onConflict: ['student_id', 'fee_head_id', 'period_key', 'is_late_fee'] });

        if (transInvItemInsertErr) {
            return { error: 'Error updating transport invoice items: ' + transInvItemInsertErr.message}
        }
    }
